...
public class AdminController {
	...
	// ===== ADMIN LOGIN (In-Memory Authentication) =====
	...
	public String adminLogin(@ModelAttribute("adminLogin") AdminLogin login,
		...
		try {
			// Authenticate admin via Spring Security's in-memory AuthenticationManager
	...
	public String userLogin(@ModelAttribute("userLogin") UserLogin login,
		...
		try {
			// Authenticate user via Spring Security's DB-backed AuthenticationManager
			...
			// Fetch user data and orders for the dashboard
	...
	public String searchHandler(@RequestParam("productName") String name,
		...
		List<Orders> orders = this.orderServices.getOrdersForUser(user);
		model.addAttribute("orders", orders);
		model.addAttribute("name", user != null ? user.getUname() : "User");
